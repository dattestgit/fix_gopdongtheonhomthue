...
BEGIN  
  ...
  BEGIN
     SELECT  CONVERT(NVARCHAR(250),'') AS HostName, CONVERT(VARCHAR(50),'') AS UserID,   
		 ...
		 MAX(T00.CurrencyID) AS CurrencyID,   
		 MAX(T00.ExchangeRate) AS ExchangeRate,  
		 ...
		 SUM(T00.Quantity) AS CQuantity,   
		 MAX(T00.UnitPrice) AS UnitPrice,  
		 SUM(T00.Amount) AS OAmount,  
		 SUM(T00.CAmount) AS CAmount,      
		 ...
		 CONVERT(VARCHAR(50),'') AS PaymentMethodID, MAX(T00.InventoryName) AS TransDesc,  
		 ...
		 MAX(T00.InventoryName) AS VATTransDesc  
	  INTO  #D06P3811_Temp_GrTax
	  FROM  D95T5000 T00 WITH(NOLOCK)  
	  WHERE  T00.VoucherID = @InputInvoiceID  
	  GROUP BY	T00.RefNo,T00.RefDate,T00.SerialNo,T00.VoucherID,VatGroup
